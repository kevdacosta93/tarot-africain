 ...
 *   rendrait le total exact.
 * - A chaque pli, un joueur qui peut jouer une carte plus forte que la
 *   meilleure carte du pli en cours est oblige de le faire ("monter").
 *   S'il n'a aucune carte capable de monter, il joue ce qu'il veut.
 * - L'Excuse vaut, au choix du joueur qui la joue, soit la plus forte
 *   carte (22, "maxi"), soit la plus faible (0, "mini"). Ce choix n'est
 *   libre que si le joueur entame le pli ; jouee en cours de pli, elle
 *   compte forcement comme "maxi" (c'est toujours une carte qui "monte").
 ...
 *   annonce et ses plis reels. Chaque joueur commence avec 10 vies.
 ...
 *   tournant a gauche a chaque manche.
 *
 ...
 * ecartees), et le donneur tourne d'un cran a chaque manche (pas
 * seulement a chaque cycle).
 */
...
class GameRoom {
  ...
  _currentBestEffective() {
    if (this.currentTrick.cards.length === 0) return null;
    return Math.max(...this.currentTrick.cards.map((c) => c.effectiveValue));
  }

  legalPlays(playerId) {
    const hand = this.hands[playerId] || [];
    if (this.phase !== 'playing' || this.turnId !== playerId) return [];
    const best = this._currentBestEffective();
    if (best === null) {
      // Le joueur entame : toutes les cartes sont jouables.
      return hand.map((c) => ({ cardId: c.id, freeDeclaration: c.type === 'excuse' }));
    }
    const qualifyingNumeric = hand.filter((c) => c.type === 'number' && c.value > best);
    const excuse = hand.find((c) => c.type === 'excuse');
    if (qualifyingNumeric.length > 0 || excuse) {
      const legal = qualifyingNumeric.map((c) => ({ cardId: c.id, freeDeclaration: false }));
      if (excuse) legal.push({ cardId: excuse.id, freeDeclaration: false, forcedDeclaration: 'maxi' });
      return legal;
    }
    return hand.map((c) => ({ cardId: c.id, freeDeclaration: false }));
  }
  ...
  playCard(playerId, cardId, declaration) {
    ...
    if (!choice) return { ok: false, error: 'Carte non jouable (regle : il faut monter si possible).' };
    ...
    let decl = null;
    if (card.type === 'excuse') {
      if (choice.forcedDeclaration) {
        decl = choice.forcedDeclaration;
      } else {
    ...
    // Pli complet : determiner le vainqueur.
    ...
    const handsRemaining = hand.length > 0 || Object.values(this.hands).some((h) => h.length > 0);
    if (handsRemaining) {
      this.currentTrick = { cards: [], leaderId: winner.playerId };
      this.turnId = winner.playerId;
      return { ok: true };
    }
  ...
  stateFor(playerId) {
    ...
    base.canPlayExcuseFree =
      this.phase === 'playing' && this.turnId === playerId && this.currentTrick.cards.length === 0;
    return base;
  }
}
